"""A2A AgentExecutor 구현 - Reviewer Agent"""
...
class ReviewerAgentExecutor(AgentExecutor):
    ...
    A2A 프로토콜에 맞게 ReviewerAgent를 래핑합니다.
    """
    ...
    async def execute(
        ...
        if not code:
            raise ServerError(error=InternalError(message="리뷰할 코드가 없습니다"))
