...
| 1.3 | 2025-02-03 | 🔴 설계 재검토: 외부 학습 vs 내부 학습 분석 추가 |

---

## 🔴 2025-02-03: 설계 재검토 및 문제점 분석

### 발견된 문제

1. **curiosity_queue 실패율 81%** (151 failed / 187 total)
2. **learned 항목들의 definition_strength가 0.3~0.5로 낮음**
3. **definition_text가 저장되지 않는 경우 많음**

### 근본적 설계 문제: 원래 의도와 현재 구현의 불일치

#### 원래 Baby AI 철학 (Task.md에서)
```
"LLM 지식은 그대로 활용, 그 위에 발달적 메커니즘 추가"
"Baby"는 지식이 없다는 의미가 아님
```

#### 현재 구현의 문제
| 원래 의도 | 현재 구현 (문제) |
|-----------|-----------------|
| CuriosityEngine = 작업 복잡도 기반 **내재적 동기** | 외부 개념 탐험으로 변질 |
| MemorySystem = **경험** 기록 및 재생 | 외부 지식 학습 시도 |
| 수면 모드 = **기억 정리** 및 강화 | 새로운 개념 학습 시도 |

### 핵심 통찰: 학습해야 할 것 vs 하지 말아야 할 것

```
┌─────────────────────────────────────────────────────────┐
│              학습해야 할 것 vs 하지 말아야 할 것           │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ✅ 학습해야 할 것 (Baby AI 고유):                       │
│  ├─ 사용자와의 대화에서 나온 고유 개념 (비비, 엄마)       │
│  ├─ 감정적으로 중요한 경험                              │
│  ├─ 사용자 선호도 및 패턴                               │
│  └─ 자아 정체성 (나는 비비, 아기 AI)                    │
│                                                         │
│  ❌ 학습하면 안 되는 것 (LLM이 이미 앎):                 │
│  ├─ 일반 지식 (algorithm, recursion)                   │
│  ├─ 프로그래밍 개념                                     │
│  └─ 백과사전적 정보                                     │
│                                                         │
└─────────────────────────────────────────────────────────┘
...
### 왜 "algorithm" 학습이 실패하는가?

1. **LLM이 이미 알고 있음** - Claude/GPT는 algorithm을 완벽히 이해함
2. **중복 학습 시도** - 이미 아는 것을 다시 배우려 함
3. **불필요한 리소스 낭비** - 웹 검색, 패턴 매칭 등

### 수정 방향 제안

#### Option 1: autonomous-exploration 비활성화/수정
- 외부 개념 탐험 중단
- 사용자 경험 기반 학습으로 전환

#### Option 2: 수면 모드 v7 재설계
- 외부 학습 ❌
- 해마 기억 재활성화 ✅ (오늘 경험 재생)
- 시냅스 정리 ✅ (약한 연결 제거)
- 정체성 강화 ✅ (비비 개념 강화)

#### Option 3: curiosity_queue 목적 변경
- "모르는 것 배우기" → "사용자가 언급한 새로운 개념 이해하기"

### TODO: 수정 작업
- [ ] autonomous-exploration에서 web_search 방법 제거 또는 제한
- [ ] curiosity 생성 로직에서 LLM 기본 지식 필터링
- [ ] 사용자 대화 기반 호기심 생성으로 전환
- [ ] 수면 모드를 해마 기억 강화 중심으로 재설계
