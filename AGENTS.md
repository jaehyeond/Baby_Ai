...
- **다음 스텝 (다양성 먼저 — 사용자·기기 의존)**:
  ...
  - **다음 스텝**: (a) `sleep_distill_job`을 **라이브 트리거**(consolidate 훅 or schedule)에 연결 → 실 대화·비전이 그래프 채우면 코어가 야간마다 자동 성장. (b) 실 Quest 다양장면 수집(데이터 병목 해소, 하드웨어). (c) effective-rank collapse 상시감시. — ⇒ **자기학습 체인 end-to-end 검증 완료, 유일 병목=데이터 밀도.**
