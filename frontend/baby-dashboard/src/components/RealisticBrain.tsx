...
import { useRef, useState, useMemo, Suspense, useCallback, Fragment } from 'react'
...
import { Brain, Loader2, ZoomIn, ZoomOut, RotateCcw, Eye, MessageCircle, ChevronDown, ChevronUp, Sparkles, Link, ArrowRight } from 'lucide-react'
...
function ThoughtProcessPanel({
  ...
  // Sort each group by intensity desc
  ...
  return (
    <div className="absolute bottom-4 left-4 right-4 max-w-[380px] bg-slate-900/95 backdrop-blur-md rounded-xl border border-violet-500/30 overflow-hidden">
...
export function RealisticBrain({
  ...
  const { activeRegions, spreadingRegions, waveCount, heatmapRegions, isReplaying, activationContext, thoughtProcess } = useNeuronActivations()
  ...
  return (
    <div className={`relative ${fullScreen ? 'h-full' : 'h-[500px]'} ${className}`}>
      ...
      {selectedRegion && (
        <div className="absolute bottom-14 right-4 bg-slate-900/90 backdrop-blur rounded-lg px-4 py-3 border border-slate-700/50 max-w-xs z-10">
      ...
      {/* v23: Thought Process Panel - shows conversation + brain's thinking process */}
      <ThoughtProcessPanel
